...
namespace ai_chat_sdk{
                                                ...
                                                std::function<void(const std::string&, bool)> callback)
    {
        ...
        Json::StreamWriterBuilder writerBuilder;
        std::string requestBodyStr = Json::writeString(writerBuilder, requestBody);
        INFO("GeminiProvider::sendMessageStream: requestBodyStr = {}", requestBodyStr); // 打印请求体
        ...
        req.content_receiver = [&](const char* data, size_t len, size_t offset, size_t totalLength) -> bool {
            ...
            INFO("GeminiProvider sendMessageStream buffer = {}", buffer); // 打印接收到的数据
                ...
                if(chunk.compare(0, 6, "data: ") == 0){
                    ...
                    if(Json::parseFromStream(reader, modelDataStream, &modelDataJson, &errs)){
                        if(modelDataJson.isMember("choices") && 
                            ...
                            WRN("GeminiProvider::sendMessageStream: parse modelDataJson failed, err = {}", errs);
                            return false;
                        }
                    } 
                }
        ...
        if(!streamFinished){
            WRN("GeminiProvider::sendMessageStream: stream not finished");
            callback("", true);
        }
        return fullResponse;
    }
